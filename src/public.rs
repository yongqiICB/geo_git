...
use crate::{db::r#impl::Db, gui::public::Config, lexer::Cursor, parser::r#impl::StringParser};

pub fn parse(x: String, cfg: Config) -> Db {
...
fn pp() {
    ...
    let mut file =
        std::fs::File::open(prj_rt.join(PathBuf::from_str("test/squeeze.txt").unwrap())).unwrap();
    let mut raw_text = String::with_capacity(256);
    file.read_to_string(&mut raw_text).unwrap();

    let db = parse(raw_text, Config::default());
...
fn real_test() {
    ...
    let prj_rt = project_root::get_project_root().unwrap();
    let mut file =
        std::fs::File::open(prj_rt.join(PathBuf::from_str("test/formal.txt").unwrap())).unwrap();
    let mut raw_text = String::with_capacity(256);
    file.read_to_string(&mut raw_text).unwrap();
