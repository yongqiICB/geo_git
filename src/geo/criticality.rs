 ...
 * methods:
    * `build()`:
    `@iter`: the iterator of a sequence of numbers.
    `@colorgrad`: a color generator with presets.
    * `get_color()`: generate the color according to the `value`'s position between `[min,max]`,
We return you a neat color generated by colorgrad.
    `@value`: value.
 */
